...
## process a subset of the data: skipped for now due to max memory error
```{r}
...
## process the data in batches: Skip this
```{r}
# For TMA1
...
# Set a stricter memory limit
gc()
memory.limit(size=16000)  # Set to 14GB, adjust as needed

# Bin the data at 1 m/z resolution
bin_width <- 1
mz_range <- range(mz(TMA1_aligned), mz(TMA2_aligned), na.rm = TRUE)
bin_breaks <- seq(floor(mz_range[1]), ceiling(mz_range[2]), by=bin_width)

# Function to preprocess data
preprocess_data <- function(data, breaks) {
  data %>%
    bin(breaks=breaks, units="mz") %>%  # Explicitly specify "mz" units
    normalize(method="tic")
}

# Process TMA1
cat("Processing TMA1\n")
TMA1_processed <- preprocess_data(TMA1_aligned, bin_breaks)
save(TMA1_processed, file="TMA1_preprocessed.RData")
rm(TMA1_aligned)
gc(full=TRUE)

# Process TMA2
cat("Processing TMA2\n")
TMA2_processed <- preprocess_data(TMA2_aligned, bin_breaks)
save(TMA2_processed, file="TMA2_preprocessed.RData")
rm(TMA2_aligned)
gc(full=TRUE)
...
# Create and save TMA1 data frame
TMA1_df <- create_df(TMA1_processed)
save(TMA1_df, file="TMA1_df.RData")
rm(TMA1_processed, TMA1_df)
gc(full=TRUE)

# Create and save TMA2 data frame
TMA2_df <- create_df(TMA2_processed)
save(TMA2_df, file="TMA2_df.RData")
rm(TMA2_processed, TMA2_df)
gc(full=TRUE)

cat("Processing complete. Data saved as RData files.\n")
